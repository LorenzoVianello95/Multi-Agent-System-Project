...
public class UAV implements Steppable{
	...
	public LinkedList<WorldCell> targetCells;
	...
	private AgentAction nextAction(Ignite ignite){
		...
		//if not, continue to move toward the target
			...
			// aggiungo all insieme da evitare tutte quelle celle confinanti con la mia attuale cella tipo acqua o burned
			try{			
				WorldCell dx=(WorldCell)ignite.forest.field[(int) j][(int) y];
				Int2D dxi= new Int2D((int)j,(int)y);
				if(!this.knownCells.contains(dx) && !this.avoidCells.contains(dxi) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(dxi);
						}
			...
			try{			
				...
				if(!this.knownCells.contains(dxdwn) && !this.avoidCells.contains(dxdwni) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(dxdwni);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{
				...
				if(!this.knownCells.contains(dwn) && !this.avoidCells.contains(dwni) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(dwni);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{			
				...
				if(!this.knownCells.contains(sxdwn) && !this.avoidCells.contains(sxdwni) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(sxdwni);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{
				...
				if(!this.knownCells.contains(sx) && !this.avoidCells.contains(sxi) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(sxi);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{			
				...
				if(!this.knownCells.contains(sxup) && !this.avoidCells.contains(sxupi) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(sxupi);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{
				...
				if(!this.knownCells.contains(up) && !this.avoidCells.contains(upi) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(upi);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			...
			try{			
				...
				if(!this.knownCells.contains(dxup) && !this.avoidCells.contains(dxupi) && (cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.WATER))) {
					this.avoidCells.addLast(dxupi);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
...
/*
			...
			try{			
				WorldCell dx=(WorldCell)ignite.forest.field[(int) j][(int) y];
				if(!this.knownCells.contains(dx) && !this.targetCells.contains(dx) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(dx);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento alla destra in basso di quello visitato
			try{			
				WorldCell dxdwn=(WorldCell)ignite.forest.field[(int) j][(int) i];
				if(!this.knownCells.contains(dxdwn) && !this.targetCells.contains(dxdwn) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(dxdwn);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento in basso a quello visitato
			try{
				WorldCell dwn=(WorldCell)ignite.forest.field[(int) x][(int) i];
				if(!this.knownCells.contains(dwn) && !this.targetCells.contains(dwn) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(dwn);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento alla sinistra in basso di quello visitato
			try{			
				WorldCell sxdwn=(WorldCell)ignite.forest.field[(int) g][(int) i];
				if(!this.knownCells.contains(sxdwn) && !this.targetCells.contains(sxdwn) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(sxdwn);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento alla sinistra di quello visitato
			try{
				WorldCell sx=(WorldCell)ignite.forest.field[(int) g][(int) y];
				if(!this.knownCells.contains(sx) && !this.targetCells.contains(sx) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(sx);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento alla simistra in alto di quello visitato
			try{			
				WorldCell sxup=(WorldCell)ignite.forest.field[(int) g][(int) h];
				if(!this.knownCells.contains(sxup) && !this.targetCells.contains(sxup) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(sxup);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento in alto di quello visitato
			try{
				WorldCell up=(WorldCell)ignite.forest.field[(int) x][(int) h];
				if(!this.knownCells.contains(up) && !this.targetCells.contains(up) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(up);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
			// aggiungo all insieme da visitare l'elemento alla destra in alto di quello visitato
			try{			
				WorldCell dxup=(WorldCell)ignite.forest.field[(int) j][(int) h];
				if(!this.knownCells.contains(dxup) && !this.targetCells.contains(dxup) && !cell.type.equals(CellType.WATER)) {
					this.targetCells.addLast(dxup);
						}
				}catch(ArrayIndexOutOfBoundsException e){}
*/	
	...
	private void selectTask(Ignite ignite) {
		...
		for(int i=1; i<taskDisp.length; i++){
...
/*
		// secondo me migliore come metodo perchè i primi 3 sicuramente verranno collocati nell modo migliore, ma 
		// dopo ci sono problemi con gli altri percè non posso costruire retrieve...
		LinkedList listTask= (LinkedList)ignite.tasks.clone();
		if(listTask.isEmpty())
				{
				if(this.x==30 && this.y==30){System.err.println("senza task");}
				else{
					Int2D midC= new Int2D(30,30);
					taskCloser=new Task(midC,1);
					this.mode=1;
		...
		for(int i=0; i<ignite.tasks.size(); i++){
			Task newTask = (Task)listTask.getFirst();
		...
		ignite.tasks.removeFirstOccurrence(taskCloser);
		}
*/
/*
		int[] taskDisp= retrieveAgents(ignite);
		...
		int indexT=0;
		int valueT=taskDisp[0];
		Task taskCloser=ignite.tasks.get(0);
		Double3D myPos = new Double3D(this.x, this.y,this.z);
		Double3D otherPos = new Double3D(taskCloser.centroid.x, taskCloser.centroid.y,this.z);
		double distmin= myPos.distance(otherPos);
		
		for(int i=1; i<taskDisp.length; i++){
			if (taskDisp[i]<valueT){
					indexT=i;
					valueT=taskDisp[i];
					taskCloser=ignite.tasks.get(i);
					otherPos = new Double3D(taskCloser.centroid.x, taskCloser.centroid.y,this.z);
					distmin= myPos.distance(otherPos);					
					}
			else if(taskDisp[i]==valueT){
					Task taskMom=ignite.tasks.get(i);
					Double3D otherPosMom = new Double3D(taskMom.centroid.x, taskMom.centroid.y,this.z);
					double distMom= myPos.distance(otherPosMom);
					if(distMom<distmin){
							indexT=i;
							valueT=taskDisp[i];
							taskCloser=taskMom;
							otherPos = otherPosMom;
							distmin= distMom;
							}						
					}
