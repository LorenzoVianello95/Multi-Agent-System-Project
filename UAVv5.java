...
public class UAV implements Steppable{
	...
	private AgentAction nextAction(Ignite ignite){
		...
		//if not, continue to move toward the target
			...
			if (cell.type.equals(CellType.NORMAL)){
						for(int i=-1;i<1;i++){
							for(int j=-1;j<0;j++){
					try{
								Int2D c= null;
								...
								if(cell.y>= this.myTask.centroid.y){
									c= new Int2D((int)x-i,(int)y+j);
									ci=(WorldCell)ignite.forest.field[(int) x-i][(int)y+j];
									System.err.println("A");
										}
								else if(cell.y< this.myTask.centroid.y){
									c= new Int2D((int)x+j,(int)y+i);
									ci=(WorldCell)ignite.forest.field[(int) x+j][(int)y+i];
									System.err.println("B");
										}
									if(!this.knownCells.contains(ci) && !this.targetCells.contains(c) && !cell.type.equals(CellType.WATER) && ((int)c.distance(this.myTask.centroid)==(int)this.myTask.radius-1)) {
						this.targetCells.addLast(c);
							}
					...
					try{
								...
								if(cell.y>= this.myTask.centroid.y){
									...
									if(!this.knownCells.contains(ci) && !this.targetCells.contains(c) && !cell.type.equals(CellType.WATER) && ((int)c.distance(this.myTask.centroid)==(int)this.myTask.radius || (int)c.distance(this.myTask.centroid)==(int)this.myTask.radius+1) ) {
						this.targetCells.addLast(c);
							}
					}catch(ArrayIndexOutOfBoundsException e){}
									}
			...
			if(cell.type.equals(CellType.BURNED) || cell.type.equals(CellType.EXTINGUISHED) && false){
			...
			//TODO maybe, you can share the knowledge about the just extinguished cell here!
					...
					try{
						...
						if(taskDisp[ignite.tasks.indexOf(this.myTask)]>1){
								System.err.println(this.id+"xx"+taskDisp[ignite.tasks.indexOf(this.myTask)]);
								this.mode=0;
								this.myTask= new Task(new Int2D(30,30),1);
	...
	private void selectTask(Ignite ignite) {
		...
		try{
			...
			try{this.target = new Double3D(taskCloser.centroid.x, taskCloser.centroid.y+(int)this.myTask.radius, z);}catch(ArrayIndexOutOfBoundsException e){this.target = new Double3D(taskCloser.centroid.x, taskCloser.centroid.y-(int)this.myTask.radius, z);}
...
*/
		...
		this.target = new Double3D(newTarget.getX(), newTarget.getY(), z);
			...
			try{this.target = new Double3D(this.myTask.centroid.x, this.myTask.centroid.y+(int)this.myTask.radius, z);}catch(ArrayIndexOutOfBoundsException f){//this.target = new Double3D(this.myTask.centroid.x, this.myTask.centroid.y-(int)this.myTask.radius, z);
					}
		}
	...
	try{
		...
		if(r==10 && otherId!=this.id && !packet.payload.rIDs.contains(this.id) && taskPos!=null && this.myTask!=null){
				...
				if(taskPos==this.myTask.centroid && modeOth <= this.mode){
							this.mode++;
							if(this.mode>1){this.mode=0; this.myTask= new Task(new Int2D(30,30),1);}
						System.err.println("qq"+this.id+ " mode "+ this.mode+ " target "+this.myTask.centroid);
							}
				packet.payload.rIDs.add(this.id);
